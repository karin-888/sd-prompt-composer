...
CATEGORY_BASES = (
    "https://ai-nante.com/category/ai-image-generator/girl-art-design/",
    "https://ai-nante.com/category/ai-image-generator/clothing-prompts/",
    "https://ai-nante.com/category/ai-image-generator/scene-assets/",
    "https://ai-nante.com/category/ai-image-generator/character-pose-prompts/",
    "https://ai-nante.com/category/ai-image-generator/prompting-technique/",
)
...
def is_plausible_tag(tag: str) -> bool:
    return tag_text_utils.is_valid_english_prompt_tag(
        tag,
        max_len=MAX_TAG_LEN,
        max_commas=MAX_TAG_COMMAS,
    )


def sanitize_entry_jp(tag: str, jp: str) -> str:
    jp = tag_text_utils.clean_catalog_jp_label(jp)
    jp = tag_text_utils.sanitize_tag_jp(tag, jp) or jp
    if jp and tag_text_utils.is_catalog_label_id(jp):
        jp = ""
    return jp or tag
...
def list_category_articles(max_pages: int = 12) -> List[str]:
    """Collect prompt article URLs across every prompt-related category."""
    found: List[str] = []
    seen = set()
...
def parse_cards(chunk: str) -> List[Dict]:
    ...
    for jp, imgattrs, tagbody in CARD_RE.findall(chunk):
        ...
        if tag_text_utils.is_catalog_label_id(jp_text) and not tag_text_utils.is_valid_english_prompt_tag(tag):
            continue
        if not is_plausible_tag(tag):
            continue
        jp_text = sanitize_entry_jp(tag, jp_text)
        items.append(
            {
                "tag": tag,
                "jp": jp_text,
...
SECTION_IMG_RE = re.compile(r'<img[^>]+src="([^"]+)"[^>]*>', re.IGNORECASE)


def first_image_url_in_chunk(chunk: str) -> str:
    """Return the URL of the first content image in a section chunk (skip avatars/icons)."""
    for src in SECTION_IMG_RE.findall(chunk or ""):
        if not src:
            continue
        low = src.lower()
        if any(skip in low for skip in ("/avatar", "icon-", "/assets/icons/", "data:image/svg")):
            continue
        return src.strip()
    return ""
...
def parse_article(url: str, html: str) -> Dict:
    ...
    section_image_by_group: Dict[str, str] = {}
    entry_groups: Dict[str, str] = {}

    for group_name, chunk in split_by_headings(html):
        chunk_image = first_image_url_in_chunk(chunk)
        if chunk_image and group_name not in section_image_by_group:
            section_image_by_group[group_name] = chunk_image
        ...
        for card in parse_cards(chunk):
            ...
            if tag not in entry_groups:
                rec["group"] = group_name
                entry_groups[tag] = group_name
            else:
                rec["group"] = entry_groups[tag]
            by_tag[tag] = rec
    ...
    for rec in by_tag.values():
        ...
        if not is_plausible_tag(tag):
            continue
        rec["jp"] = sanitize_entry_jp(tag, rec.get("jp") or "")
        if not (rec.get("image_url") or "").strip():
            fallback = section_image_by_group.get(rec.get("group") or "")
            if fallback:
                rec["image_url"] = fallback
                rec["image_url_fallback"] = True
        entries.append(rec)
...
def run_import(dry_run: bool = False, limit: Optional[int] = None) -> Dict:
    ...
    if not dry_run:
        ...
        manifest = {
            ...
            "source_categories": list(CATEGORY_BASES),
